...
LoadBalancer 타입 Service를 만들면 내부적으로 보통 아래 순서가 일어난다.

1. **Kubernetes가 처리**
- ClusterIP 생성
- NodePort 자동 할당

2. **Azure(AKS 연동 컴포넌트)가 처리**
- Azure Load Balancer 생성
- Frontend IP 할당(환경에 따라 공인/사설)
- LB Rule이 `FrontendIP:ServicePort -> NodeIP:NodePort`로 연결

즉, `Service(type=LoadBalancer)` 하나를 만들었을 때 별도의 `NodePort Service` 리소스를 추가로 만드는 것이 아니라,
동일 Service가 NodePort를 포함하고 Azure LB가 그 NodePort로 붙는 구조다.
