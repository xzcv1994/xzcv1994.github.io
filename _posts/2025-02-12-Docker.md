...
## **Docker를 관통하는 핵심 개념**
{: .mt-4 .mb-2}
Docker는 "파일 시스템 스냅샷 위에서 프로세스를 실행하는 기술"이라고 볼 수 있다.
즉, Docker 이미지는 **"특정 프로세스를 실행하기 위해 필요한 파일들과 설정만 모아둔 실행 환경 패키지"**라고 이해하면 가장 정확하다.

##### **파일 시스템 스냅샷이란?**
{: .mt-4 .mb-2}
Docker Image는 특정 시점의 루트 파일 시스템 상태를 저장한 결과물이다.
...
이 구조 자체가 이미지의 스냅샷이며, 기본적으로 읽기 전용이다.

##### **컨테이너 실행 시 실제로 일어나는 일**
{: .mt-4 .mb-2}
컨테이너를 실행하면 Docker는 아래 순서로 동작한다.

1. 이미지 레이어를 합쳐 루트 파일 시스템을 구성한다.
2. 그 위에 writable layer(쓰기 가능한 레이어) 하나를 추가한다.
3. `ENTRYPOINT`로 지정된 프로세스를 실행한다.

예를 들어 아래 Dockerfile이면:

```dockerfile
ENTRYPOINT ["java", "-jar", "app.jar"]
```

컨테이너 내부에서는 결국 아래 프로세스가 실행된다.

```text
/usr/bin/java -jar /app/app.jar
```

즉, "미리 준비된 파일 시스템 환경 안에서 프로세스 하나를 실행하는 것"이 컨테이너의 본질이다.

##### **VM과의 차이**
{: .mt-4 .mb-2}
- VM: OS 전체 부팅, 커널/가상 하드웨어 포함
- Docker: 호스트 커널 공유, 파일 시스템/프로세스 중심 격리

그래서 컨테이너는 훨씬 빠르게 시작할 수 있다.

##### **정확한 한 줄 정리**
{: .mt-4 .mb-2}
Docker는 특정 파일 시스템 상태를 루트 디렉토리처럼 구성하고, 그 안에서 프로세스를 실행하는 기술이다.
...
- ENTRYPOINT : 컨테이너가 시작될 때 반드시 실행할 기본 실행 파일/명령을 지정한다.
...
## **ENTRYPOINT는 무엇인가?**
{: .mt-5 .mb-2}
`ENTRYPOINT`는 컨테이너 시작 시 실행할 메인 프로세스를 고정하는 명령이다.

```dockerfile
ENTRYPOINT ["java", "-jar", "app.jar"]
```

위처럼 설정하면 컨테이너가 시작될 때 항상 `java -jar app.jar`가 실행된다.

##### **ENTRYPOINT와 CMD 차이**
{: .mt-4 .mb-2}
- `ENTRYPOINT`: 컨테이너의 고정 실행 명령(뼈대)
- `CMD`: 기본 인자 또는 기본 명령(옵션)
...
동작:
- 기본 실행: `java -jar app.jar --spring.profiles.active=prod`
- `docker run 이미지 --server.port=8081` 처럼 인자를 넘기면 `CMD`가 대체된다.

##### **언제 ENTRYPOINT를 쓰면 좋은가**
{: .mt-4 .mb-2}
- 컨테이너가 항상 동일한 프로세스로 시작되어야 할 때
- 실행 명령은 고정하고, 실행 옵션만 유동적으로 바꾸고 싶을 때
- 애플리케이션 컨테이너의 진입점을 명확히 강제하고 싶을 때
